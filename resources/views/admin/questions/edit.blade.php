...
@section('title', 'Edit Question - Admin Panel')
...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <form action="{{ route('admin.questions.update', $question->question_id) }}" method="POST" id="editQuestionForm">
            @csrf
            @method('PUT')
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('article_id') is-invalid @enderror" 
                        ...
                        @foreach($articles as $article)
                            <option value="{{ $article->article_id }}" {{ old('article_id', $question->article_id) == $article->article_id ? 'selected' : '' }}>
                                {{ $article->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" 
                        ...
                        <option value="single" {{ old('type', $question->type) == 'single' ? 'selected' : '' }}>Single Choice</option>
            ...
            <div class="mb-3">
                ...
                <div id="optionsContainer">
                    ...
                    {{-- Iterate valid options --}}
                    @foreach($options as $key => $value)
                        <div class="option-group">
                            <div class="d-flex gap-2">
                                <input type="text" class="form-control option-key" 
                                       placeholder="Option Identifier (e.g. A/B/C)" 
                                       value="{{ old("options.$key", $key) }}">
                                <input type="text" class="form-control option-value flex-grow-1" 
                                       placeholder="Option Content" 
                                       value="{{ old("options.$key", $value) }}">
                                @if($loop->index >= 4)
            ...
            <!-- Correct Answer (Adapt array to string display) -->
            <div class="mb-3">
                <label for="answer" class="form-label">Correct Answer <span class="text-danger">*</span></label>
                <div class="form-text mb-2" id="answerHint">
                    {{ $question->type == 'single' ? 'Enter correct option identifier (single choice: one value, e.g. A)' : 'Enter correct option identifiers (multiple choice: comma separated, e.g. A,C)' }}
                </div>
                <input type="text" class="form-control @error('answer') is-invalid @enderror" 
                       id="answer" name="answer" 
                       value="{{ old('answer', is_array($question->answer) ? implode(',', $question->answer) : $question->answer) }}" 
                       required>
                @error('answer')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            {{-- Hidden field: Store options in JSON format --}}
...
<script>
    ...
    // Bind events to existing remove buttons
    ...
    // Form submission handling (JSON adaptation)
    document.getElementById('editQuestionForm').addEventListener('submit', function(e) {
        ...
        // Save options to hidden field (JSON format)
        document.getElementById('optionsJson').value = JSON.stringify(options);
        
        // Process answer: submit as array
        const answerField = document.createElement('input');
        answerField.type = 'hidden';
        answerField.name = 'answer_array';
        answerField.value = JSON.stringify(answerArray);
        this.appendChild(answerField);
    });

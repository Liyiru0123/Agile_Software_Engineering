...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        {{-- Critical Fix: Changed $article->id to $article->article_id --}}
        <form action="{{ route('admin.articles.update', $article->article_id) }}" method="POST" id="editArticleForm">
            @csrf
            @method('PUT')
            <div class="row mb-3">
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('subject') is-invalid @enderror" 
                        ...
                        <option value="Civil Engineering" {{ old('subject', $article->subject) == 'Civil Engineering' ? 'selected' : '' }}>Civil Engineering</option>
                        <option value="Mathematics" {{ old('subject', $article->subject) == 'Mathematics' ? 'selected' : '' }}>Mathematics</option>
                        <option value="Computer Science" {{ old('subject', $article->subject) == 'Computer Science' ? 'selected' : '' }}>Computer Science</option>
                        <option value="Mechanical Engineering" {{ old('subject', $article->subject) == 'Mechanical Engineering' ? 'selected' : '' }}>Mechanical Engineering</option>
                        <option value="Mechanical Engineering with Transportation" {{ old('subject', $article->subject) == 'Mechanical Engineering with Transportation' ? 'selected' : '' }}>Mechanical Engineering with Transportation</option>
                    </select>
                    @error('subject')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('level') is-invalid @enderror" 
                        ...
                        <option value="Easy" {{ old('level', $article->level) == 'Easy' ? 'selected' : '' }}>Easy</option>
                        <option value="Intermediate" {{ old('level', $article->level) == 'Intermediate' ? 'selected' : '' }}>Intermediate</option>
                        <option value="Advanced" {{ old('level', $article->level) == 'Advanced' ? 'selected' : '' }}>Advanced</option>
                    </select>
                    @error('level')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" class="form-control @error('word_count') is-invalid @enderror" 
                           id="word_count" name="word_count" value="{{ old('word_count', $article->word_count) }}" min="0">
                    @error('word_count')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
